...
#### 0. ✅ **ЗАВЕРШЕНО** - БЕЗОПАСНОСТЬ: Полная изоляция данных между отделами (Multi-tenancy Security Audit)
**Статус**: ✅ **КРИТИЧЕСКИЕ УЯЗВИМОСТИ ИСПРАВЛЕНЫ** (2025-10-29)

**Результаты аудита**:
- **Проверено**: 15 API модулей, 153 endpoints, ~8,000 строк кода
- **Найдено**: 5 CRITICAL, 2 HIGH, 4 MODERATE уязвимостей
- **Исправлено**: Все 5 CRITICAL уязвимостей
- **Тесты**: 22 security tests созданы
- **Документация**: `docs/MULTI_TENANCY_SECURITY.md`
- **Security Grade**: A- (94.8% endpoints secure)

**Критические уязвимости (ВСЕ ИСПРАВЛЕНЫ ✅)**:
1. ✅ attachments.py - Все 6 endpoints позволяли cross-department access
2. ✅ analytics.py - Отсутствовали импорты UserRoleEnum, status (все endpoints ломались)
3. ✅ budget.py - Отсутствовал импорт UserRoleEnum (все endpoints ломались)
4. ✅ budget.py /init - USER мог инициализировать бюджет других отделов
5. ✅ budget.py /copy-from - USER мог копировать бюджет других отделов

**Решение**:

**Backend - Аудит моделей данных:**
- [x] ✅ Проверить наличие `department_id` во ВСЕХ таблицах (кроме organizations)
  ...
  - [x] ✅ `attachments` - через expense.department_id (FIXED - все 6 endpoints)
  - [x] ✅ `users` - имеет department_id
  - [x] ✅ `departments` - собственная таблица
  - [x] ✅ `organizations` - ОБЩИЕ (нет department_id) ✅
  - [x] ✅ `audit_logs` - department_id nullable (OK)
  - [x] ✅ Все таблицы проверены: 15 с department_id, 7 через relationships

**Backend - Аудит API endpoints (Row Level Security):**
- [x] ✅ `/api/v1/expenses/*` - ВСЕ 11 endpoints SECURE
  - [x] ✅ GET /expenses - USER видит только свой отдел
  - [x] ✅ GET /expenses/{id} - проверка принадлежности к отделу
  - [x] ✅ POST /expenses - автоматическая установка department_id
  - [x] ✅ PUT /expenses/{id} - запрет изменения чужих записей
  - [x] ✅ DELETE /expenses/{id} - запрет удаления чужих записей

- [x] ✅ `/api/v1/attachments/*` - ИСПРАВЛЕНО: добавлена проверка через expense
  - [x] ✅ Все 6 endpoints теперь проверяют department через parent expense

- [x] ✅ `/api/v1/budget-categories/*` - ВСЕ 9 endpoints SECURE

- [x] ✅ `/api/v1/contractors/*` - 6/8 SECURE, 2 MODERATE (bulk ops)

- [x] ✅ `/api/v1/budget/plans/*` - ИСПРАВЛЕНО: /init и /copy-from
  - [x] ✅ USER больше не может указать чужой department_id

- [x] ✅ `/api/v1/budget/versions/*` - ВСЕ 27 endpoints SECURE

- [x] ✅ `/api/v1/employees/*` - ВСЕ 8 endpoints SECURE

- [x] ✅ `/api/v1/payroll/*` - ВСЕ 26 endpoints SECURE
  - [x] ✅ /payroll/plans - фильтрация по department_id
  - [x] ✅ /payroll/actuals - фильтрация по department_id
  - [x] ✅ /payroll/analytics/* - фильтрация по department_id

- [x] ✅ `/api/v1/kpi/*` - 22/24 SECURE, 2 MODERATE (MANAGER filter)
  - [x] ✅ /kpi/goals - фильтрация по department_id
  - [x] ✅ /kpi/employee-kpis - фильтрация по department_id
  - [x] ✅ /kpi/analytics/* - фильтрация по department_id

- [x] ✅ `/api/v1/forecast/*` - 6/7 SECURE, 1 MODERATE

- [x] ✅ `/api/v1/analytics/*` - ИСПРАВЛЕНЫ импорты, 9/11 SECURE
  - [x] ✅ FIXED: добавлены UserRoleEnum и status imports
  - [x] ✅ /analytics/dashboard - фильтрация по department_id
  - [x] ✅ /analytics/budget-execution - фильтрация по department_id
  - [x] ✅ /analytics/by-category - фильтрация по department_id
  - [ ] ⚠️ /analytics/plan-vs-actual - HIGH: data leakage (отложено)
  - [x] ✅ /analytics/payment-calendar - фильтрация по department_id

**Backend - Проверка ролей:**
- [x] ✅ USER role:
  - [x] ✅ Видит ТОЛЬКО данные своего отдела
  - [x] ✅ Не может изменить department_id при создании записей
  - [x] ✅ Не может получить доступ к данным других отделов

- [x] ✅ MANAGER role:
  - [x] ✅ Может фильтровать по department_id (опционально)
  - [x] ✅ Видит ВСЕ отделы если фильтр не указан

- [x] ✅ ADMIN role:
  - [x] ✅ Может видеть все отделы
  - [x] ✅ Может управлять всеми данными

**Backend - Написать тесты безопасности:**
- [x] ✅ Создан `test_multi_tenancy_security.py` (22 теста)
  - [x] ✅ Тест: USER отдела A не может получить expense отдела B
  - [x] ✅ Тест: USER отдела A не может изменить expense отдела B
  - [x] ✅ Тест: USER отдела A не может удалить expense отдела B
  - [x] ✅ Тест: USER не может создать запись с чужим department_id
  - [x] ✅ Тесты для attachments, budget, payroll, kpi, employees
  - [x] ✅ Тесты для ADMIN/MANAGER ролей

**Frontend - Аудит компонентов:**
- [ ] ⏭️ Отложено на следующий спринт (backend security - приоритет)
...
**Документация:**
- [x] ✅ Создан документ `docs/MULTI_TENANCY_SECURITY.md`
  - [x] ✅ Политика безопасности между отделами
  - [x] ✅ Список всех проверенных endpoints (153 endpoints)
  - [x] ✅ Результаты тестирования (5 CRITICAL fixed)
  - [x] ✅ Рекомендации для разработчиков

- [ ] ⏭️ Обновить `CLAUDE.md` с примерами (отложено)

**Метрики успеха (ДОСТИГНУТЫ ✅):**
- ✅ 100% таблиц имеют department_id (15/15 + 7 через relationships)
- ✅ 94.8% endpoints проверяют department_id (145/153 SECURE)
- ✅ 22 security tests созданы
- ✅ Zero CRITICAL vulnerabilities в production
- ✅ Документация актуализирована

**Статус**: ✅ **ЗАВЕРШЕНО** - Все критические уязвимости исправлены (2025-10-29)
**Оставшиеся задачи**: 2 HIGH + 4 MODERATE (отложены на следующий спринт)

---

#### 1. 🔴 КРИТИЧНО: Dashboard - План/Факт с учетом ФОТ
...
- [x] Backend: Исправить `/api/v1/analytics/dashboard` endpoint
  - Добавить в расчет факта сумму из PayrollActual (аналогично плану)
  - Файл: `backend/app/api/v1/analytics.py:80-88`
  - Код: добавить запрос к PayrollActual и включить в total_actual
- [x] Тестирование: Проверить корректность расчетов план/факт с ФОТ
- [x] Документация: Обновить описание endpoint в OpenAPI

#### 2. 🔴 КРИТИЧНО: График "Исполнение бюджета" - фильтры и привязки
...
**Решение**:
- [x] Backend: Добавить фильтры в `/api/v1/analytics/budget-execution`
  - Добавить параметр `department_id` (опционально)
  - Файл: `backend/app/api/v1/analytics.py:210-246`
- [x] Frontend: BudgetPlanVsActualWidget
  - Убрать год из заголовка (использовать динамический год из фильтра)
  - Добавить пропс `departmentId` и передавать в API
  - Файл: `frontend/src/components/dashboard/widgets/BudgetPlanVsActualWidget.tsx`
- [x] Frontend: Проверить работу BudgetExecutionProgressWidget и BudgetDeviationHeatmap
  - Убедиться что baseline версия установлена для года
  - Добавить улучшенные сообщения об ошибках
  - Файлы: `frontend/src/components/dashboard/widgets/BudgetExecutionProgressWidget.tsx`, `BudgetDeviationHeatmap.tsx`

#### 3. 🟡 СРЕДНИЙ: Прогноз в календаре оплат
**Проблема**: Прогноз в календаре оплат не работает корректно.

**Решение**:
- [x] Backend: Проверить PaymentForecastService
  - Убедиться что forecast генерируется корректно
  - Проверить методы: `simple_average`, `moving_average`, `seasonal`
  - Файл: `backend/app/services/forecast_service.py`
- [x] Frontend: PaymentCalendarPage
  - Проверить отображение прогнозов (серым цветом)
  - Улучшить визуализацию прогнозов vs фактических платежей
  - Добавить легенду для прогнозов
  - Файл: `frontend/src/pages/PaymentCalendarPage.tsx`

#### 4. 🟢 НИЗКИЙ: Прогноз расходов с AI
**Проблема**: Прогноз расходов использует простые статистические методы, нужно добавить AI для более релевантного прогноза.

**Решение**:
- [ ] Backend: Интеграция AI API
  - Создать новый сервис `AIForecastService`
  - Интеграция с https://api.vsegpt.ru/v1
  - Модель: `openai/gpt-5-mini`
  - API Key: `sk-or-vv-e3d1733d71226f04372d5bd90843b3615228e15f4bc3936371f97198d986b01a`
  - Файл: `backend/app/services/ai_forecast_service.py` (создать)
- [ ] Backend: AI-enhanced forecast endpoint
  - Endpoint: `POST /api/v1/forecast/ai-generate`
  - Параметры: year, month, department_id, use_ai=true
  - Автоматическое заполнение комментариев на основе истории заявок
  - Учет паттернов и сезонности через AI
- [ ] Frontend: Добавить опцию AI прогноза
  - Checkbox "Использовать AI прогноз" в ForecastPage
  - Индикатор загрузки AI прогноза
  - Сравнение AI vs статистический прогноз
  - Файл: `frontend/src/pages/ForecastPage.tsx`
- [ ] Тестирование: Сравнение точности AI vs статистика
- [ ] Документация: Описание AI прогноза

#### 5. 🔴 КРИТИЧНО: Расширенная аналитика - создать страницу
**Проблема**: Страница "Расширенная аналитика" совсем не работает (не существует).

**Решение**:
- [ ] Backend: API endpoints для расширенной аналитики
  - `GET /api/v1/analytics/advanced/expense-trends` - тренды по категориям
  - `GET /api/v1/analytics/advanced/contractor-analysis` - анализ по контрагентам
  - `GET /api/v1/analytics/advanced/department-comparison` - сравнение отделов
  - `GET /api/v1/analytics/advanced/seasonal-patterns` - сезонные паттерны
  - `GET /api/v1/analytics/advanced/cost-efficiency` - эффективность затрат
  - Файл: `backend/app/api/v1/analytics_advanced.py` (создать)
- [ ] Frontend: Создать ExtendedAnalyticsPage
  - Многоуровневые фильтры (год, месяц, отдел, категория, контрагент)
  - Дашборды с расширенными графиками:
    - Водопадная диаграмма изменений бюджета
    - Sankey diagram потоков расходов
    - Scatter plot корреляций
    - Heatmap активности по дням недели
    - Treemap распределения расходов
  - Экспорт отчетов в Excel/PDF
  - Файл: `frontend/src/pages/ExtendedAnalyticsPage.tsx` (создать)
- [ ] Навигация: Добавить пункт меню "Расширенная аналитика"

#### 6. 🟡 СРЕДНИЙ: KPI сотрудников - UX улучшения
**Проблема**: KPI сотрудников неудобно использовать, страница слишком большая (1523 строки).

**Решение**:
- [ ] Frontend: Рефакторинг KpiManagementPage
  - Разделить на отдельные компоненты:
    - `KpiGoalsTable.tsx` - таблица целей
    - `EmployeeKpiTable.tsx` - таблица КПИ сотрудников
    - `KpiAssignmentsTable.tsx` - назначения целей
    - `KpiSummaryCards.tsx` - карточки сводки
    - `KpiCalendar.tsx` - календарь бонусов
    - `EmployeeKpiDrawer.tsx` - детали сотрудника
  - Улучшить навигацию между разделами
  - Добавить быстрые действия (bulk operations)
  - Файл: `frontend/src/pages/KpiManagementPage.tsx`
- [ ] UX: Упрощение форм
  - Пошаговый wizard для создания KPI
  - Автозаполнение базовых значений
  - Валидация в реальном времени
- [ ] Performance: Оптимизация таблиц
  - Виртуализация для больших списков
  - Мемоизация тяжелых вычислений

#### 7. 🟡 СРЕДНИЙ: Аналитика ФОТ - налоги и графики
**Проблема**: В Аналитике ФОТ нет расчета налогов (НДФЛ, страховые взносы) и мало графиков.

**Решение**:
- [ ] Backend: Добавить расчет налогов
  - НДФЛ (13%/15% прогрессивная шкала)
  - Страховые взносы (30%: ПФР 22%, ФОМС 5.1%, ФСС 2.9%)
  - Endpoint: `GET /api/v1/payroll/analytics/taxes`
  - Файл: `backend/app/api/v1/payroll.py`
- [ ] Backend: Расширить API аналитики ФОТ
  - `GET /api/v1/payroll/analytics/cost-breakdown` - детализация затрат
  - `GET /api/v1/payroll/analytics/tax-summary` - сводка по налогам
  - Включить налоги в существующие endpoints
- [ ] Frontend: PayrollAnalyticsPage - добавить графики
  - Waterfall chart: Base Salary → Bonuses → Taxes → Net Salary
  - Stacked area chart: Salary + Taxes динамика
  - Pie chart: Распределение налогов (НДФЛ, ПФР, ФОМС, ФСС)
  - Bar chart: Tax burden по сотрудникам/отделам
  - Line chart: Effective tax rate по месяцам
  - Файл: `frontend/src/pages/PayrollAnalyticsPage.tsx`
- [ ] Frontend: Добавить секцию "Налоги и взносы"
  - Карточки с итогами по налогам
  - Таблица детализации налогов
  - Прогноз налоговой нагрузки

#### 8. 🟢 НИЗКИЙ: Калькулятор НДФЛ - проверка функциональности
**Проблема**: Калькулятор НДФЛ не работает.

**Решение**:
- [ ] Frontend: Проверить NDFLCalculator компонент
  - Убедиться в корректности расчетов (13%/15% прогрессивная шкала 2024-2025)
  - Проверить учет вычетов (стандартные, социальные, имущественные)
  - Файл: `frontend/src/components/payroll/NDFLCalculator.tsx`
- [ ] Backend: Проверить NDFL calculator service
  - Валидация расчетов
  - Файл: `backend/app/utils/ndfl_calculator.py`
- [ ] Улучшения калькулятора:
