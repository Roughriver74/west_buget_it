...
- [x] ✅ `/api/v1/attachments/*` - 🔴 КРИТИЧНО исправлено (6 endpoints)
...
#### 3. ✅ 🟡 СРЕДНИЙ: Прогноз в календаре оплат ✅ (2025-10-30)
...
#### 5. ✅ 🔴 КРИТИЧНО: Расширенная аналитика - создать страницу ✅ (Проверено 2025-10-30)
...
#### 6. 🔄 🟡 СРЕДНИЙ: KPI сотрудников - UX улучшения (В ПРОЦЕССЕ - 2025-10-30)
**Статус**: 🔄 **В ПРОЦЕССЕ** - План создан, proof-of-concept готов (1/6 компонентов)
...
- [~] Frontend: Рефакторинг KpiManagementPage - **1/6 ГОТОВО**
  - [x] ✅ `KpiSummaryTab.tsx` - карточка сводки (~130 строк) - **ГОТОВО**
  - [ ] `KpiGoalsTab.tsx` - таблица целей (~350 строк)
  - [ ] `EmployeeKpiTab.tsx` - таблица КПИ сотрудников (~250 строк)
  - [ ] `KpiAssignmentsTab.tsx` - назначения целей (~250 строк)
  - [ ] `KpiCalendar.tsx` - календарь бонусов (~200 строк)
  - [ ] `EmployeeKpiDrawer.tsx` - детали сотрудника (~150 строк)
  - [ ] Обновить главный `KpiManagementPage.tsx` (1523 → ~150 строк)
...
**Примечание**: Рефакторинг частично выполнен. Создан KpiSummaryTab как proof-of-concept. Оставшиеся 5 компонентов требуют ~2.5-3 часа работы.

#### 7. 🔄 🟡 СРЕДНИЙ: Аналитика ФОТ - налоги и графики (BACKEND ГОТОВ - 2025-10-30)
**Статус**: 🔄 **В ПРОЦЕССЕ** - Backend завершен (50%), осталось Frontend
...
- [ ] Frontend: Установить ApexCharts для Waterfall
  - `npm install --save react-apexcharts apexcharts`
- [ ] Frontend: Новая вкладка "Налоги и взносы"
  - 4 карточки: НДФЛ, Взносы, Общая нагрузка, Эффективная ставка
  - Waterfall chart: Base → Bonuses → Taxes → Net
  - Stacked area chart: Salary + Taxes динамика
  - Pie chart: Распределение налогов (НДФЛ, ПФР, ФОМС, ФСС)
  - Bar chart: Tax burden по сотрудникам
  - Line chart: Effective tax rate по месяцам
  - Table: Детализация налогов
  - Файл: `frontend/src/pages/PayrollAnalyticsPage.tsx`

**Примечание**: Backend полностью готов (~2.5 часа работы). Используется существующий НДФЛ калькулятор. Осталось Frontend (~2.5-3 часа).

#### 8. ✅ 🟢 НИЗКИЙ: Калькулятор НДФЛ - проверка функциональности ✅ (Проверено 2025-10-30)
