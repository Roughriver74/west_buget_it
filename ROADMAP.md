...
#### 0. 🔴🔴🔴 КРИТИЧНО - БЕЗОПАСНОСТЬ: Полная изоляция данных между отделами (Multi-tenancy Security Audit)
...
- [ ] Проверить наличие `department_id` во ВСЕХ таблицах (кроме organizations)
  ...
  - [ ] ❓ `attachments` - проверить привязку (через expense.department_id)
  - [x] ✅ `users` - имеет department_id
  - [x] ✅ `departments` - собственная таблица
  - [x] ✅ `organizations` - ОБЩИЕ (нет department_id) ✅
  - [ ] ❓ `audit_logs` - department_id nullable (проверить фильтрацию)
  - [ ] Все остальные таблицы проверить

**Backend - Аудит API endpoints (Row Level Security):**
- [ ] `/api/v1/expenses/*` - Проверить фильтрацию по department_id
  - [ ] GET /expenses - USER видит только свой отдел
  - [ ] GET /expenses/{id} - проверка принадлежности к отделу
  - [ ] POST /expenses - автоматическая установка department_id
  - [ ] PUT /expenses/{id} - запрет изменения чужих записей
  - [ ] DELETE /expenses/{id} - запрет удаления чужих записей

- [ ] `/api/v1/budget-categories/*` - Проверить фильтрацию
  - [ ] Все операции только для своего отдела

- [ ] `/api/v1/contractors/*` - Проверить фильтрацию
  - [ ] Все операции только для своего отдела

- [ ] `/api/v1/budget/plans/*` - Проверить фильтрацию
  - [ ] Все операции только для своего отдела

- [ ] `/api/v1/budget/versions/*` - Проверить фильтрацию
  - [ ] Все операции только для своего отдела

- [ ] `/api/v1/employees/*` - Проверить фильтрацию
  - [ ] Все операции только для своего отдела

- [ ] `/api/v1/payroll/*` - Проверить все endpoints
  - [ ] /payroll/plans - фильтрация по department_id
  - [ ] /payroll/actuals - фильтрация по department_id
  - [ ] /payroll/analytics/* - фильтрация по department_id

- [ ] `/api/v1/kpi/*` - Проверить все endpoints
  - [ ] /kpi/goals - фильтрация по department_id
  - [ ] /kpi/employee-kpis - фильтрация по department_id
  - [ ] /kpi/analytics/* - фильтрация по department_id

- [ ] `/api/v1/forecast/*` - Проверить фильтрацию
  - [ ] Все операции только для своего отдела

- [ ] `/api/v1/analytics/*` - КРИТИЧНО проверить все аналитические endpoints
  - [ ] /analytics/dashboard - фильтрация по department_id
  - [ ] /analytics/budget-execution - фильтрация по department_id
  - [ ] /analytics/by-category - фильтрация по department_id
  - [ ] /analytics/plan-vs-actual - фильтрация по department_id
  - [ ] /analytics/payment-calendar - фильтрация по department_id
  - [ ] Все остальные аналитические endpoints

**Backend - Проверка ролей:**
- [ ] USER role:
  - [ ] Видит ТОЛЬКО данные своего отдела (current_user.department_id)
  - [ ] Не может изменить department_id при создании записей
  - [ ] Не может получить доступ к данным других отделов даже если передаст их ID

- [ ] MANAGER role:
  - [ ] Может фильтровать по department_id (опционально)
  - [ ] По умолчанию видит ВСЕ отделы или только свой? (определить политику)

- [ ] ADMIN role:
  - [ ] Может видеть все отделы
...
**Документация:**
- [ ] Создать документ `docs/MULTI_TENANCY_SECURITY.md`
  - [ ] Политика безопасности между отделами
  - [ ] Список всех проверенных endpoints
  - [ ] Результаты тестирования
  - [ ] Рекомендации для разработчиков

- [ ] Обновить `CLAUDE.md` с примерами безопасной реализации

**Метрики успеха:**
- ✅ 100% таблиц имеют department_id (кроме organizations)
- ✅ 100% endpoints проверяют department_id
- ✅ 100+ security tests проходят успешно
- ✅ Zero cross-department data leaks в production
