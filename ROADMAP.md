...
#### 🤖 AI-прогнозирование расходов 2.0 (v0.8.x) 🟡 Приоритет: СРЕДНИЙ
- **Цель**: Дотянуть модуль AI-прогнозирования расходов до сценариев из статьи (диапазоны, вероятности, корреляции, рекомендации) и поднять точность до 85%+
- **Задачи**:
  1. [ ] Расширить подготовку данных для AI
     - [ ] Сохранять до 24 месяцев истории, сезонные коэффициенты и аномалии (выбросы, новые проекты)
     - [ ] Добавить справочник внешних событий (проекты, найм, контракты) и связывать их с прогнозом
     - [ ] Прокидывать в AI контекст approved-плана и стат. моделей (simple/moving/seasonal)
  2. [ ] Обновить AI-промпт и backend API
     - [ ] Расширить `AIForecastResponse` (base/optimistic/pessimistic, min/max диапазоны, вероятность сценариев)
     - [ ] Возвращать отдельный блок `correlations` и `recommendations` (экономия, риски, действия)
     - [ ] Хранить метаданные прогнозов (источник суммы, confidence, версия модели) в БД
  3. [ ] Доработать UX ForecastPage
     - [ ] Показ диапазонов на графике и в таблице, сравнение с базовым статистическим прогнозом
     - [ ] Блок "Анализ корреляций" и "Рекомендации" в модальном окне AI
     - [ ] Визуализация уверенности и сценариев (progress/segment bar)
  4. [ ] Внедрить контроль качества и автоматизацию
     - [ ] Считать MAPE/SMAPE по факту, писать отчеты об отклонениях
     - [ ] Планировщик пересчета (еженощно + перед закрытием месяца) и уведомления при дрейфе
     - [ ] Подготовить датасет для fine-tuning и выделить гипотезы для доп. факторов (праздники, кризисы)
- **Результат**: AI-прогноз выдает диапазоны с вероятностями, объясняет причины через корреляции и дает actionable-рекомендации, а система отслеживает точность и пересчитывает прогнозы автоматически
- **Оценка**: 2-3 недели (Backend 1.5, Frontend 0.5, DevOps/ML 0.5)
