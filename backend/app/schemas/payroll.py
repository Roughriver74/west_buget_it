...
class EmployeeBase(BaseModel):
    ...
    # Salary calculation fields (Task 1.4: Gross ↔ Net)
    salary_type: SalaryTypeEnum = Field(
        SalaryTypeEnum.GROSS,
        description="Тип ввода оклада: GROSS (gross, до вычета НДФЛ) или NET (net, на руки)"
    )
...
class EmployeeInDB(EmployeeBase):
    ...
    # Calculated salary fields (Task 1.4: Gross ↔ Net)
    base_salary_gross: Optional[Decimal] = Field(None, description="Оклад gross (до вычета НДФЛ)")
    base_salary_net: Optional[Decimal] = Field(None, description="Оклад net (на руки после НДФЛ)")
