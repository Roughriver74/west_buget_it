...
class BankTransaction1CImporter:
    ...
    def _import_receipts(
        ...
        while True:
            try:
                ...
                # Если получили меньше batch_size, значит это последний батч
    ...
    def _process_receipt(
        ...
        # Проверить существование по external_id_1c
    ...
    def _parse_receipt_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Парсинг данных из ДанныеВыписки (если есть)
        ...
        result = {
            ...
            'business_operation': data.get('ХозяйственнаяОперация', None),  # ХозОперация для авто-категоризации
            ...
            'document_type': DocumentTypeEnum.PAYMENT_ORDER,  # Банковский документ
            'payment_source': PaymentSourceEnum.BANK  # BANK для банковских документов
        }

        # Данные из выписки (если есть)
        if statement_data:
            result.update({
        ...
        # Установить месяц/год
        if transaction_date:
            result['transaction_month'] = transaction_date.month
            result['transaction_year'] = transaction_date.year
    ...
    def _parse_statement_data(self, statement_str: str) -> Dict[str, str]:
        """
        Парсинг поля ДанныеВыписки (формат ключ=значение, разделитель \n)

        Example:
            Номер=172
            Дата=16.06.2020
            Сумма=33880.00
            ПлательщикИНН=7805343282
            ...
        """
    ...
    def _parse_cash_receipt_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = {
            ...
            'business_operation': data.get('ХозяйственнаяОперация', None),  # ХозОперация для авто-категоризации
            'document_number': data.get('Number', ''),
            'document_date': transaction_date,  # Для кассовых документов дата = дата документа
            'notes': data.get('Комментарий', ''),
            'status': BankTransactionStatusEnum.NEW,
            'document_type': DocumentTypeEnum.CASH_ORDER,  # Кассовый документ
            'payment_source': PaymentSourceEnum.CASH  # CASH для кассовых документов
        }

        # Контрагент из справочника 1С (если указан)
    ...
    def _extract_guid_from_nav_link(self, link: Optional[str]) -> Optional[str]:
        """Извлечь GUID из navigationLinkUrl, если есть"""
        if not link:
            return None
    ...
    def _apply_classification(self, transaction: BankTransaction):
        ...
        try:
            # Call classifier.classify method with correct signature
            category_id, confidence, reasoning = self.classifier.classify(
                ...
                department_id=self.department_id,
                transaction_type=transaction.transaction_type,  # Pass transaction type for better classification
                business_operation=transaction.business_operation  # HIGHEST PRIORITY: жёсткий маппинг из 1С
            )
