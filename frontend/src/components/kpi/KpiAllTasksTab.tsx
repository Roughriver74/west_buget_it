/**
 * KPI All Tasks Tab Component
 *
 * Управление всеми задачами KPI для отдела.
 */
...
interface KpiAllTasksTabProps {
  departmentId?: number
}

const KpiAllTasksTab: React.FC<KpiAllTasksTabProps> = ({ departmentId }) => {
  ...
  // Fetch all tasks for department
  const { data: tasks = [], isLoading } = useQuery({
    queryKey: ['kpi-tasks', departmentId],
    queryFn: () =>
  ...
  const { data: statistics } = useQuery({
    queryKey: ['kpi-task-statistics', departmentId],
    queryFn: () => kpiTasksApi.getKPITaskStatistics(undefined, departmentId),
    enabled: !!departmentId,
  })
  ...
  // Fetch KPI goals for new task form (for future use in dropdown)
  // const { data: kpiGoals = [] } = useQuery({
  //   queryKey: ['kpi-goals', departmentId],
  //   queryFn: () => kpiApi.listGoals({ department_id: departmentId }),
  //   enabled: !!departmentId && taskModalVisible,
  // })

  // Fetch assignments for new task form (to get employee_kpi_goal_id)
  const { data: assignments = [] } = useQuery({
    queryKey: ['kpi-assignments', departmentId],
    queryFn: () => kpiApi.listAssignments({}),
    enabled: !!departmentId && taskModalVisible,
  })
  ...
  const filteredTasks = useMemo(() => {
    return tasks.filter((task) => {
      ...
      const employeeMatch = employeeFilter === 'ALL' || task.employee_id === employeeFilter
      return statusMatch && priorityMatch && employeeMatch
    })
  ...
  const handleTaskSubmit = async () => {
    try {
      ...
      if (editingTask) {
        ...
        // Create new task
        await createTaskMutation.mutateAsync(taskData as KPITaskCreate)
      }
    } catch (error) {
        ...
        <Form form={taskForm} layout="vertical">
          {!editingTask && (
            <>
              ...
              <Form.Item
                name="employee_kpi_goal_id"
                label="Цель KPI"
                rules={[{ required: true, message: 'Выберите цель KPI' }]}
                tooltip="Выберите цель, к которой относится эта задача"
              >
                <Select
                  showSearch
                  placeholder="Выберите цель KPI"
                  optionFilterProp="children"
                >
                  {assignments.map((assignment) => (
                    <Option key={assignment.id} value={assignment.id}>
                      {assignment.goal?.name || `Цель #${assignment.goal_id}`} -{' '}
                      {employees.find((e: Employee) => e.id === assignment.employee_id)?.full_name}
                    </Option>
                  ))}
                </Select>
              </Form.Item>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item
                ...
                tooltip="Оценка сложности задачи по шкале от 1 до 10 для расчета премий"
              >
