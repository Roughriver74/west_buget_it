...
export default function CreditPortfolioPage() {
	...
	const isFinanceManager =
		user?.role === 'MANAGER' && selectedDepartment?.code === 'FIN'
	const canTriggerFTPImport = Boolean(
		user?.role === 'ADMIN' || isFinanceManager
	)
	...
	const loadTestDataMutation = useMutation({
		...
		onSuccess: data => {
			message.success(
				`Тестовые данные загружены успешно! ${data.data.organizations} организаций, ${data.data.contracts} договоров, ${data.data.receipts} поступлений, ${data.data.expenses} списаний`
			)
			...
			if (error.response?.status === 400) {
				...
				Modal.confirm({
					...
					content:
						...
						'Хотите перезагрузить тестовые данные? Это удалит существующие тестовые данные и создаст новые.',
				...
				message.error(
					error.response?.data?.detail || 'Ошибка при загрузке тестовых данных'
				)
			}
		},
	})
	...
	const triggerFtpImportMutation = useMutation({
		mutationFn: () => creditPortfolioApi.triggerImport(),
		onSuccess: data => {
			const successMessage =
				data.message ||
				`Импорт из FTP: ${data.files_processed}/${data.total_files} файлов`
			message.success(successMessage)
			const queriesToInvalidate = [
				['credit-portfolio-summary'],
				['credit-portfolio-monthly'],
				['credit-receipts'],
				['credit-expenses'],
				['credit-contracts'],
				['credit-portfolio'],
		...
		onError: (error: any) => {
			const errorMessage =
				error.response?.data?.detail ||
				error.message ||
				'Ошибка при загрузке данных из FTP'
				...
				{canTriggerFTPImport && (
					<Space>
						<Button
							...
							onClick={() => triggerFtpImportMutation.mutate()}
							loading={triggerFtpImportMutation.isPending}
						>
							Загрузить из FTP
						</Button>
						{user?.role === 'ADMIN' && (
						...
						<Col xs={24} sm={12} lg={6}>
							<Card>
								<Statistic
									...
									valueStyle={{
										color:
											(summary?.net_balance || 0) >= 0 ? '#3f8600' : '#cf1322',
									}}
								/>
							</Card>
						</Col>
