...
export default function CreditPortfolioAnalyticsPage() {
  ...
  if (!summary) {
    return (
      <div style={{ padding: 24 }}>
        <h1 style={{ marginBottom: 24 }}>Кредитный портфель - Расширенная аналитика</h1>
  ...
  const monthlyChartData = monthlyEfficiency?.map((item: any) => ({
    month: item.month,
    principal: item.principal || 0,
    interest: item.interest || 0,
    efficiency:
      item.principal + item.interest > 0
        ? ((item.principal / (item.principal + item.interest)) * 100).toFixed(1)
        : 0,
  })) || []
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        ...
        const efficiency = total > 0
          ? ((record.principal / total) * 100).toFixed(1)
          : '0.0'
      ...
      sorter: (a: any, b: any) => {
        const aEff = (a.principal + a.interest) > 0
          ? (a.principal / (a.principal + a.interest)) * 100
          : 0
        const bEff = (b.principal + b.interest) > 0
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <Card title="Помесячная эффективность погашения" style={{ marginBottom: 24 }}>
        ...
        <Row gutter={[16, 16]}>
          <Col span={24}>
            ...
            <p>
              ...
              Высокое значение указывает на высокую стоимость обслуживания долга.
            </p>
          </Col>
          <Col span={24}>
            <h4>Скорость погашения</h4>
            <p>
              Показывает как быстро погашается долг относительно полученных средств.
              Значение близкое к 100% означает равномерное погашение.
            </p>
          </Col>
          <Col span={24}>
            <h4>Эффективность погашения</h4>
            <p>
              Рассчитывается как отношение погашения тела кредита к общей сумме платежей.
              Высокая эффективность (&gt;70%) означает, что большая часть платежей идет на
              погашение основного долга, а не на проценты.
            </p>
          </Col>
        </Row>
      </Card>
