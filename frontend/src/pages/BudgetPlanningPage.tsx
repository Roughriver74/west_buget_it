...
const BudgetPlanningPage: React.FC = () => {
	...
	const [versionDetailMode, setVersionDetailMode] = useState<'view' | 'edit'>(
		'view'
	)
	...
	const departmentId =
		selectedDepartment?.id ?? user?.department_id ?? undefined
	...
	const scenarioIds = useMemo(() => {
		const base = scenarios.find(
			s => s.scenario_type === BudgetScenarioType.BASE
		)
		const optimistic = scenarios.find(
			s => s.scenario_type === BudgetScenarioType.OPTIMISTIC
		)
		const pessimistic = scenarios.find(
			s => s.scenario_type === BudgetScenarioType.PESSIMISTIC
		)
	...
	const hasEnoughScenariosForComparison =
		Object.values(scenarioIds).filter(Boolean).length >= 2
		...
		<div style={{ padding: '24px' }}>
				...
				extra={
					<Space>
						...
						<Button
							...
							disabled={
								!selectedScenario || categoriesLoading || categoriesError
							}
						>
							Калькулятор
						</Button>
			...
			>
				{!selectedScenario ? (
					...
					<LoadingState
						fullHeight={false}
						message='Загрузка версий бюджета...'
					/>
				) : versionsError ? (
					<ErrorState
						fullHeight={false}
				...
				<Form form={scenarioForm} layout='vertical'>
					...
					>
						<Select placeholder='Выберите тип'>
							<Select.Option value={BudgetScenarioType.BASE}>
								Базовый
							</Select.Option>
							<Select.Option value={BudgetScenarioType.OPTIMISTIC}>
								Оптимистичный
							</Select.Option>
							<Select.Option value={BudgetScenarioType.PESSIMISTIC}>
								Пессимистичный
							</Select.Option>
						</Select>
					</Form.Item>
			...
			<BudgetVersionComparisonModal
				...
				version1Id={
					selectedVersionsForComparison[0]
						? Number(selectedVersionsForComparison[0])
						: null
				}
				version2Id={
					selectedVersionsForComparison[1]
						? Number(selectedVersionsForComparison[1])
						: null
				}
