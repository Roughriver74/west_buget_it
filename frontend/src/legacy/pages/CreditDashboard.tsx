...
const CreditDashboard = ({ departmentId }: CreditDashboardProps) => {
  ...
  const limitBase = useMemo(() => {
    const hasOrgFilter = debouncedFilters.organizations && debouncedFilters.organizations.length > 0
    const hasBankFilter = debouncedFilters.bankAccounts && debouncedFilters.bankAccounts.length > 0
    return hasOrgFilter || hasBankFilter ? 5000 : 10000
  }, [debouncedFilters.organizations, debouncedFilters.bankAccounts])
  ...
  const expenseParams = useMemo<ExpenseQueryParams>(() => {
    ...
    if (filterPayload.date_from) params.date_from = filterPayload.date_from
    if (filterPayload.date_to) params.date_to = filterPayload.date_to
    if (filterPayload.organization) params.organization = filterPayload.organization
    if (filterPayload.bank_account) params.bank_account = filterPayload.bank_account
    return params
  }, [filterPayload, limitBase, departmentId])
  ...
  const expenseDetailsQuery = useQuery({
    ...
    queryFn: () => expenseDetailsAPI.getAll({ limit: 15000, department_id: departmentId }),
    placeholderData: keepPreviousData,
...
import { motion } from 'framer-motion'
import { useQuery, useQueryClient, keepPreviousData } from '@tanstack/react-query'
import { receiptsAPI, expensesAPI, expenseDetailsAPI } from '@/legacy/services/api'
import type {
  Expense,
  ExpenseDetail,
  Receipt,
...
const CreditDashboard = ({ departmentId }: CreditDashboardProps) => {
  ...
  if (hasError) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4 text-center bg-white rounded-xl shadow-sm">
        <p className="text-gray-600">Не удалось загрузить данные дашборда. Попробуйте еще раз.</p>
        <button
          onClick={handleRefresh}
          className="px-4 py-2 rounded-lg bg-blue-500 text-white hover:bg-blue-600 transition-colors"
...
const CreditDashboard = ({ departmentId }: CreditDashboardProps) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      transition={{ duration: 0.5 }}
      className="p-5 bg-white rounded-2xl shadow-sm space-y-6"
      id="dashboard-content"
    >
      <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
